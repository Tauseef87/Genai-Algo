...
class Citation(BaseModel):
    path: Annotated[
        str,
        ...,
        "The VERBATIM path name of a SPECIFIC source referred to generate the answer.",
    ]
...
def add_context_system_prompt(ctx: RunContext[KnowledgeDeps]) -> str:
    ...
    docs = base_retriever.invoke(ctx.deps.query)
    res = format_docs(docs)
    return "<context>\n" + res + "\n</context>"
